...
class TakeHearingViewModel with ChangeNotifier {
  bool loader = false;
  ...
  void sendOnTap(Complain complain, ActionTypePref typePref, ActionViewPref viewPref) {
    ...
    var context = navigatorKey.currentState?.context;
    if (context == null) return;
    var message1 = 'Please write a note for hearing'.translate;
    ...
    _takeHearingApi(complain, typePref, viewPref);
  }

  Future<void> _takeHearingApi(Complain complain, ActionTypePref typePref, ActionViewPref viewPref) async {
    _loader(true);
    var context = navigatorKey.currentState!.context;
    var body = _takeHearingBody(context, complain);
    var proofFiles = Provider.of<ProofFilesViewModel>(context, listen: false).proofFiles;
    var response = await sl<OfficerActionRepository>().takeHearing(body, proofFiles);
    if (response) backToPrevious();
    if (response && viewPref == ActionViewPref.details_view) backToPrevious();
    _loader(false);
  }

  Map<String, String> _takeHearingBody(BuildContext context, Complain complain) {
    var filenames;
